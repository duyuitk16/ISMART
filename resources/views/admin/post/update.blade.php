...
@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            ...
            <div class="card-body">
                <form enctype="multipart/form-data" action="{{ url("admin/post/edit/$this_post->id") }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="content" class="form-control" id="contentPost" cols="30" rows="5">{{ $this_post->content }}</textarea>
                    </div>
                    @error('content')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                        ...
                        <select class="form-control" id="belongsTo" name="belongsTo">
                            ...
                            @foreach ($post_cats as $post_cat)
                                ...
                                <option value="{{ $post_cat->id }}"
                                    {{ $this_post->post_cat_id == $post_cat->id ? 'selected' : '' }}>
                                    {{ $post_cat->name }}
                                </option>
                            @endforeach
                        </select>
