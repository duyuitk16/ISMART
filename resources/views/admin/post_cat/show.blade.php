        ...
        <div class="row">
            ...
            <div class="col-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="analytic">
                            <a href="{{ url('admin/post/cat/list?status=active') }}" class="text-primary">Hoạt
                                động<span class="text-muted">({{ $count[0] }})</span></a>
                            <a href="{{ url('admin/post/cat/list?status=trash') }}" class="text-primary">Thùng rác<span
                                ...
                                @if (count($post_cats) > 0)
                                    ...
                                    @foreach ($post_cats as $post_cat)
                                        ...
                                        <tr>
                                            ...
                                            @endphp
                                            <td>{{ $post_cat->name }}</td>
                                            <td>{{ $post_cat->status == 'on' ? 'Công khai' : 'Chờ duyệt' }}</td>
                                            @if ($post_cat->user_id)
                                                <td>{{ $post_cat->user->name }}</td>
                                            @else
                                                <td class="text-danger">Người tạo đã bị xóa vĩnh viễn</td>
                                            @endif
                                            <td>{{ $post_cat->created_at->format('H:i:s d-m-Y') }}</td>
                                            <td>
                                                @php
                                                    if ($status == 'trash') {
                                                ...
                                                @endphp
                                                <a href="{{ url("admin/post/cat/$url_type/$post_cat->id") }}"
                                                ...
                                                @endphp
                                                <a href="{{ url("admin/post/cat/$delete_type/$post_cat->id") }}"
